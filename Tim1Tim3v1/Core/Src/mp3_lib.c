...
volatile uint8_t mp3_cmd_buf[10] = {0x7E, 0xFF, 0x06, 0x00, 0x00, 0x0, 0x0, 0x00, 0x00, 0xEF};
...
void MP3_send_cmd (uint8_t cmd, uint16_t high_arg, uint16_t low_arg) {
	...
	//HAL_UART_Transmit_IT(&huart1, mp3_cmd_buf, (uint16_t)10);
	HAL_UART_Transmit_IT(&huart1, (uint8_t *)&mp3_cmd_buf, 10);
